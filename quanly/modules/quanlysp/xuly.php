...
elseif(isset ($_POST['suasanpham'])){
    ...
    if($hinhanh!= ''){
       ...
       move_uploaded_file($hinhanh_tmp,'uploads/'.$hinhanh);
       // từ sang sử lý vào uploasd tim đến hình ảnh vs id tương ứng để xóa hình ảnh cũ và thêm hình ảnh mới vào
    ...
    while($row = mysqli_fetch_array($query)){
        unlink('uploads/'.$row['hinhanh']);
        // từ sang sử lý vào uploasd tim đến hình ảnh vs id tương ứng để xóa 
    }
    $sql_xoa = "DELETE FROM tbl_sanpham where id_sanpham='".$id."'";
    mysqli_query($mysqli,$sql_xoa);
    header('Location:../../index.php?action=quanlysp=them');
}
